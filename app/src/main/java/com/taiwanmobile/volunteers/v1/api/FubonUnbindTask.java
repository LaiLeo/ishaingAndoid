...
public class FubonUnbindTask extends BaseRequestTask{
	public static final String TAG = "FubonUnbindTask";

	public FubonUnbindTask(Context c) {
		super(c);
	...
	private Request buildFubonUnbindRequest(){
	...
	public Response executeRequest(Request request){
		...
		try {
			...
			e.printStackTrace();
			sendResult(UserProfileEditFragment.MSG_FUBON_UNBIND_FAILED);
		}
		return null;
	}
	...
	public Boolean doInBackground(Void... arg0) {
		...
		Request request = buildFubonUnbindRequest();
	...
	public boolean handleResponse(Response response){
		...
		try {
			...
			if (code == 200) {
				sendResult(UserProfileEditFragment.MSG_FUBON_UNBIND_SUCCESS);
				return true;
			}else{
				sendResult(UserProfileEditFragment.MSG_FUBON_UNBIND_FAILED);
			}
